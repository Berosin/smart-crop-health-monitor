...
SCORE_COLORS = ["#CE8C82", "#D6A34B", "#7FA687"]
...
def render() -> None:
    ...
    callout(
        f"{icon_html('dashboard', size=18)}Showing dummy data. Real statistics "
        "will appear once the database layer is connected."
    )
    footer()
...
def _kpi_row(total, healthy, diseased, avg_score, high_risk) -> None:
    ...
    with c4:
        _tile("Avg Health Score", str(avg_score), "#D6A34B", "#1C2E20",
              "out of 100")
    with c5:
        _tile("High-Risk Plants", str(high_risk), "#B5564B", "#7C3730",
