...
def render() -> None:
    ...
    if model is None:
        callout(
            "⚠️ **Model not found.** Train the model first using "
            "`python -m src.model_training --data-dir data/samples` "
            "or place a trained model at `models/crop_disease_model.h5`."
        )
        footer()
        return

    callout(
        f"✅ **Model loaded** — {model.name} with {len(class_names)} classes: {', '.join(class_names)}"
    )
...
def _render_result(pred: dict) -> None:
    ...
    # Banner
    sev_color, sev_action = SEVERITY_META.get(pred["severity"], ("#9e9e9e", "Unknown"))
    ...
    st.markdown(
        ...
        <div style="background:{banner_bg};border-left:5px solid {banner_border};
          ...
          <div style="font-size:1.5rem;font-weight:700;color:{pred['color'] if 'color' in pred else CLASS_COLORS.get(pred['disease'], '#333')}">
            {pred['disease']}
          </div>
          <div style="font-size:.85rem;color:#666">{sev_action}</div>
        </div>
        """,
