...
FACTOR_META = {
    "temperature":   {"emoji": "🌡️", "label": "Temperature",   "nudge": 2},
    "humidity":      {"emoji": "💧", "label": "Humidity",      "nudge": 5},
    "soil_moisture": {"emoji": "🌱", "label": "Soil moisture", "nudge": 5},
    "rainfall":      {"emoji": "🌧️", "label": "Rainfall",      "nudge": 2},
}
        ...
        for col, key in zip(cols * 2, factor_order):
            with col:
                ...
                inputs[key] = st.number_input(
                    f"{FACTOR_META[key]['emoji']} {FACTOR_META[key]['label']} ({spec['unit']})",
...
def _assess(crop: str, inputs: dict) -> dict:
    ...
    for key in ("temperature", "humidity", "soil_moisture", "rainfall"):
        ...
        if opt_min <= value <= opt_max:
            status, color, note = "Optimal", "#66bb6a", "Within ideal range"
            ...
            status, color, note = "High", "#ef5350", f"Above ideal ({opt_min}–{opt_max}{unit})"
            suboptimal += 1
            norm = (value - low) / (high - low) if high != low else 0.75
        else:
            # Outside the safe band entirely (very low or very high).
            status, color, note = "Extreme", "#c62828", "Outside safe range"
            suboptimal += 1
...
def _render_risk_banner(results: dict) -> None:
    ...
    bg = "#fff5f5" if results["suboptimal"] >= 2 else "#e8f5e9"
    st.markdown(
        f"""
        <div style="background:{bg};border-left:5px solid {color};
          ...
          <div style="display:flex;justify-content:space-between;align-items:center">
            ...
            <div style="text-align:right">
              <div style="font-size:.8rem;color:#555">Env. health score</div>
...
def _render_factor_table(results: dict) -> None:
    ...
    for f in results["factors"]:
        rows += (
            ...
            f"<td style='color:#777'>{f['note']}</td>"
            f"</tr>"
        )
    st.markdown(
